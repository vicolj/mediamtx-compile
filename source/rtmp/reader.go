...
func h265TrackFromConfig(data []byte) (*format.H265, error) {
...
type Reader struct {
	...
	h265RTMPCallback   OnDataH26xFunc
	h265ExtCallback    OnDataH26xFunc
}
		...
		case *message.Video:
			...
			if msg.Type == message.VideoTypeConfig && videoTracks[0] == nil {
				...
				case message.CodecH265:
					videoTracks[0], err = h265TrackFromConfig(msg.Payload)
				default:
					err = fmt.Errorf("unsupported video codec: %d", msg.Codec)
				}
				if err != nil {
					return nil, nil, err
				}
...
func (r *Reader) OnDataH265(track *format.H265, cb OnDataH26xFunc) {
	// Store the extended RTMP callback
	r.h265ExtCallback = cb
	
	// The combined handler is set up by OnDataH265RTMP
	// If OnDataH265RTMP hasn't been called yet, set up a basic handler
	trackID := r.videoTrackID(track)
	if r.onVideoData[trackID] == nil {
		r.onVideoData[trackID] = func(msg message.Message) error {
			switch msg := msg.(type) {
			case *message.VideoExFramesX:
				if r.h265ExtCallback == nil {
					return nil
				}
				var au h264.AVCC
			...
			case *message.VideoExCodedFrames:
				...
				r.h265ExtCallback(msg.DTS+msg.PTSDelta, au)
			}
			return nil
		}
	}
...
// OnDataH265RTMP sets a callback that is called when H265 data from traditional RTMP is received.
func (r *Reader) OnDataH265RTMP(track *format.H265, cb OnDataH26xFunc) {
	trackID := r.videoTrackID(track)
	
	// Store the traditional RTMP callback
	r.h265RTMPCallback = cb
	
	// Set up a combined handler that can handle both traditional and extended RTMP
	r.onVideoData[trackID] = func(msg message.Message) error {
		switch msg := msg.(type) {
		case *message.Video:
			if msg.Codec != message.CodecH265 {
				return nil
			}

			switch msg.Type {
			case message.VideoTypeConfig:
				// For H.265 traditional RTMP, we don't send config as a separate AU.
				// The config information is already stored in the track format,
				// and will be prepended to keyframes by the format processor.
				return nil

			case message.VideoTypeAU:
				if r.h265RTMPCallback == nil {
					return nil
				}
				var au h264.AVCC
				err := au.Unmarshal(msg.Payload)
				...
				r.h265RTMPCallback(msg.DTS+msg.PTSDelta, au)
			}
